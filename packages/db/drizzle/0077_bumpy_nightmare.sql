...
CREATE TABLE `__new_page_component` (
	...
	FOREIGN KEY (`external_service_id`) REFERENCES `external_service`(`id`) ON UPDATE no action ON DELETE cascade,
	FOREIGN KEY (`external_service_component_id`) REFERENCES `external_service_component`(`id`) ON UPDATE no action ON DELETE cascade,
	FOREIGN KEY (`group_id`) REFERENCES `page_component_groups`(`id`) ON UPDATE no action ON DELETE set null,
	CONSTRAINT "page_component_type_check" CHECK(("__new_page_component"."type" = 'monitor' AND "__new_page_component"."monitor_id" IS NOT NULL AND "__new_page_component"."external_service_id" IS NULL AND "__new_page_component"."external_service_component_id" IS NULL) OR ("__new_page_component"."type" = 'static' AND "__new_page_component"."monitor_id" IS NULL AND "__new_page_component"."external_service_id" IS NULL AND "__new_page_component"."external_service_component_id" IS NULL) OR ("__new_page_component"."type" = 'external' AND "__new_page_component"."monitor_id" IS NULL AND "__new_page_component"."external_service_id" IS NOT NULL))
);

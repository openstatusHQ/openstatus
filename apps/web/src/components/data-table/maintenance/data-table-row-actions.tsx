...
import { selectMaintenanceSchema } from "@openstatus/db/src/schema";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export function DataTableRowActions<TData>({
  ...
  const maintenance = selectMaintenanceSchema.parse(row.original);
  ...
  async function onDelete() {
    startTransition(async () => {
      try {
        ...
        await api.maintenance.delete.mutate({ id: maintenance.id });
        toastAction("deleted");
        router.refresh();
        setAlertOpen(false);
      } catch {

...
description:
  We have deployed a simple Hono server on various cloud providers to test the
  latency of each of them using OpenStatus.
author:
...
We wanted to test the latency of Cloudflare Workers, Fly, Koyeb, Railway and
Render using OpenStatus.

<Tweet id="1752762168041439731" />

It's a good way to dogfood our own product and improve it.

For this test, we used a basic Hono server that returns a simple response.
...
app.get("/", (c) => { return c.text( "Just return the desired http status code,
...
We have deployed this application on the cheapest or free tier available from
each provider.
...
<div className="mt-4">
  <SimpleChart staticFile="/assets/posts/monitoring-latency/cloudflare.json" />
</div>
...
We haven't encountered any cold start issues.
...
<div className="mt-4">
  <SimpleChart staticFile="/assets/posts/monitoring-latency/fly.json" />
</div>
...
It’s slow but according to the logs our machine start in `1.513643778s`
...
The DNS is fast. We are trying to connect to a region in the same data center as
our checker, but we are slowed down by the cold start of our machine, which
explains the high TTFB.

We use it in production, and the machine never sleeps, yielding much better
results.
...
<div className="mt-4">
  <SimpleChart staticFile="/assets/posts/monitoring-latency/koyeb.json" />
</div>
...
I have deployed our app in the Frankfurt datacenter
...
<div className="mt-4">
  <SimpleChart staticFile="/assets/posts/monitoring-latency/railway.json" />
</div>
...
<div className="mt-4">
  <SimpleChart staticFile="/assets/posts/monitoring-latency/render.json" />
</div>
...
inflection point between cold and warn.

## Conclussion

If you are only looking from a latency perspective, Cloudflare Workers is the
best choice. It’s fast worldwide and we haven’t encountered any cold start
issues.

But picking a cloud provider is not only about latency. It’s also about the DX.
We have to take into account the ease of use, the pricing of each of them.

...
of interest. We were trending on GitHub and the numbers of users and the created
...
- We were breaking the `/monitor/[id]/data` data-table. We are storing the
  metadata `res.text()` inside of tinybird and were fetching
  `SELECT * from monitors WHERE …`. This means that if the text is html we will
  store the html of the page, which is fine. But that means also, that we
  quickly reached the 100MB limit of result length send whenever you where
...
P.S. We are generating your workspace-slug by merging two random names. NotRoman
came by and share us his slug:

![wet-grandmother slug](/assets/posts/first-48h/funny-workspace-slug.png)

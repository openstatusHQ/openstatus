...
1$/100k Qstash
messages which would lead us to a approx. 10$/day for the current free tiers.

### Actions taken

But due to the high cost of our current infrastructure, **we had to downgrade
all monitors to a single region**, specifically `/regions/auto`, in order to
stay within a reasonable budget (dividing it by 18). You can edit the region for
each monitor. We apologize for any inconvenience caused and will update you on
the new conditions once everything is normalized.
...
P.S. We are generating your workspace-slug by merging two random names. Share
yours on [Discord](https://openstatus.dev/discord) - it's fun to watch!

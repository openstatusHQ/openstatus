...
description: An 8.5MB Docker image that runs uptime checks on your home network.
author:
...
We just pushed the first version of **Private Regions** at OpenStatus - and I had to test it on my old **Raspberry Pi 3**.
...
Because I hadn’t used this Raspberry Pi for development before, I freshly installed **Docker** using the official Debian repository instructions: 
...
Compare that to 1GB+ for some competitors (yes, they pack more features, but still 🤯).
...
And that’s it - my **Raspberry Pi (2016)** is officially back in action.  
Running quietly next to my receiver, it’s now part of the openstatus network, checking endpoints from my home.

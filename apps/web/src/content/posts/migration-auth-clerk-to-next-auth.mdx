...
now NextAuth we can seed the DB and login with an email (magic link gets printed
in the console) within less than a minute.

<Tweet id="1790147744964780230" />

> Not gonna lie, we still have some more improvements to make, like adding a
> better Tinybird setup, but saving it up for later! ✍️

## Why did we start with Clerk? 🧑‍💻

When we started OpenStatus, we needed a way to authenticate users. We wanted to
make it easy for users to sign up to our platform. Clerk was a good choice for
...
All the other information of the user was stored in Clerk.
...
NextAuth to store the social image of the user).
...
Now you might wonder: **How did we migrate the data?** We did not.

While Clerk has all the informations for the social `accounts` of every user, we
...
We have been using Clerks UI for a long time and swapped it with custom

...
`getMonitorSummary` returns a single aggregate over the window and `TimeRange` caps at 14 days. For a per-day series (e.g. to render status bars), use `getMonitorDailySummary` below.

## Get Monitor Daily Summary

Get per-day status buckets for one or more monitors over the last N days (max 45). Each bucket is tagged with its `monitorId`, so you can render a status bar per monitor. This is the recommended way to feed the `status-bar` / `status-calendar` registry components.
...
Each `MonitorDailyStat` has `monitorId`, `day` (RFC 3339, UTC midnight), and the `bigint` counts `count`, `ok`, `degraded`, `error`. Days with no checks are omitted — fill gaps client-side. Daily data is retained for 45 days.

<Aside type="note">The REST endpoint `GET /v1/monitor/{id}/summary` also returns a daily series, but only `{ ok, count, day }` for a single monitor. Prefer `getMonitorDailySummary` — it is multi-monitor and includes `degraded`/`error`.</Aside>

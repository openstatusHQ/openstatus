...
The SDK talks to the API over a fetch-based Connect transport (`@connectrpc/connect-web`), so it runs on any runtime with a global `fetch` — no HTTP/2 or `node:*` modules required:
...
The default client works on the edge as-is. The one caveat is that `@connectrpc/connect-web` issues requests with `redirect: "error"`, which `workerd` doesn't implement. If you hit `The redirect mode 'error' is not supported`, pass a transport with a redirect-tolerant `fetch` (the SDK re-exports `createAuthInterceptor` and the service descriptors for this):

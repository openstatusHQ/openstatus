...
- The AI assistant's docs knowledge base (the `search_docs`/`get_doc_page` and `search_content`/`get_content_page` tools) queries the public `https://www.openstatus.dev` site by default. Set `OPENSTATUS_WEB_BASE_URL` if you mirror the marketing site and want the assistant to read your own copy.
...
2.  **Configure your environment**
    ...
    ```
    Open `.env.docker` in a text editor. Set every value marked `[REQUIRED]`, including `NEXT_PUBLIC_URL` for the dashboard and `STATUS_PAGE_URL` for the public status-page service. For a complete setup, review the file for other variables like OAuth providers or email services.
    ...
    # Public base domain routed to the status-page container. Slug pages use
    # subdomains, for example https://acme.status.example.com.
    STATUS_PAGE_URL=https://status.example.com

    # Stable hostname customers use as their custom-domain CNAME target.
    CUSTOM_DOMAIN_CNAME_TARGET=status.example.com
    ```

    You'll fill in the Tinybird tokens in Part 2, once the container is running.

    If you plan to connect MCP clients (Claude, Cursor, ChatGPT) with OAuth, also set the two origins the flow redirects through. With `NODE_ENV=production` they default to openstatus Cloud, which would send your users to `app.openstatus.dev` to approve a connection:
    ```env
    # Public origin of the API server (authorization server and /mcp resource).
    OAUTH_ISSUER=http://localhost:3001
    # Public origin of the dashboard, which serves the consent screen.
    DASHBOARD_URL=http://localhost:3002
    ```
    Use your real hostnames once the stack sits behind a reverse proxy. The [MCP server reference](/docs/reference/mcp-server#self-hosting) has the details.
...
-   **Status Pages:** `http://localhost:3003`
    ...
    With `STATUS_PAGE_URL=https://status.example.com`, a page whose slug is
    `acme` is published at `https://acme.status.example.com`. Configure a
    wildcard DNS record for `*.status.example.com`; Caddy's catch-all below
    provisions an individual certificate for each saved slug.

    For Caddy, set the hostname users should target with
    `CUSTOM_DOMAIN_CNAME_TARGET=status.example.com`, then enable On-Demand TLS:

    ```caddyfile
    {
      on_demand_tls {
        ask http://127.0.0.1:3003/api/caddy/ask
      }
    }
    ...
    Keep dashboard, API, ingest, and workflow hostnames in their own exact
    Caddy site blocks. The catch-all handles only other hostnames. Before Caddy
    issues a certificate, it calls the `ask` endpoint; openstatus returns `200`
    only when the hostname is a saved custom domain or a valid configured slug
    hostname. Unknown domains are denied.

    A customer then points a CNAME such as `status.acme.com` to
    `status.example.com`. Caddy provisions HTTPS on the first request and
    forwards the original hostname to the status-page container. The dashboard
    checks the saved hostname and its live DNS through
    `status.example.com/api/domain-check`, then marks it ready when both match.

    The three `*_VERCEL` values are only for deployments whose status-page app
    is itself hosted on Vercel and should attach domains through Vercel's API.
    Leave all three blank for Docker, Coolify, Kubernetes, or another reverse
    proxy. When they are blank, openstatus stores the custom domain but leaves
    DNS and TLS to your infrastructure.
...
10. **Deploy a probe**
    ...
    -   `OPENSTATUS_KEY` — the key you copied from the dashboard. Some older docs call this `OPENSTATUS_TOKEN`; that name is not read by the probe, which exits on startup with `OPENSTATUS_KEY is required` if the variable is unset.

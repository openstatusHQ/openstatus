...
- `/openstatus unsubscribe [status-page-url]` — unsubscribe. The URL is optional when the channel is subscribed to exactly one page; with several, the command lists them and asks you to name one.
...
1. **Check the channel** — make sure the bot has been added to it. Run `/openstatus subscriptions` there, or re-run `/openstatus subscribe <status-page-url>` to join it. Private channels can't be self-joined; use `/invite @openstatus`.

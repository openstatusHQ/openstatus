...
+------------------------------------------------+
      |
      | (Alerts: Email, WhatsApp Slack, etc.) 🔔
      |
+-----+-----+
| Your Team |
+-----------+
...
- **Alerting**: The system of notifying a team or individual when downtime is detected. Alerts can be sent via email, Slack, WhatsApp, or other communication channels.
...
- **Region**: A geographic place where openstatus runs checks from (e.g. `iad`, `fra`, `sin`). Region is one layer in a stack with *probe* and *location*; see [Probes, Locations, and Regions](/docs/concept/probes-and-locations) for the distinction. The [Location Reference](/docs/reference/location) has the full list and the IPs to allowlist.

...
Both metrics matter, but for different reasons. This page explains what each one measures and how to read them; if you are looking for target numbers, see [what is a good server response time?](/guides/what-is-a-good-response-time).
...
**Response time is the sum, not the first byte.** The connection phases — DNS,
connect, and TLS — are the network cost of getting there before your application does
any work. Add TTFB and transfer and you have total response time, which is what a
user waits through.
...
- **[What is a good server response time?](/guides/what-is-a-good-response-time)** — target numbers for both the server and the browser half, judged at the right percentile.

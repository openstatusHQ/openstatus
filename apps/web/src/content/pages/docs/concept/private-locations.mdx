...
1. You deploy the openstatus probe container — typically `ghcr.io/openstatushq/private-location:latest` — somewhere inside the network that needs to be monitored.
...
The consequence is that you only need **outbound** network access from the probe to the openstatus ingest endpoint, `openstatus-private-location.fly.dev`, on port 443. No inbound firewall rules, no exposed ports, no reverse tunnel — the same shape as any other agent that calls home.

If you self-host openstatus, point the probe at your own ingest server with the `OPENSTATUS_INGEST_URL` environment variable — see [self-hosting openstatus](/docs/guides/self-hosting-openstatus).
...
- **Egress only.** Because the connection is outbound, an egress firewall that blocks `openstatus-private-location.fly.dev:443` will silently stop the probe from running checks. The probe will appear "connected" briefly and then go quiet.

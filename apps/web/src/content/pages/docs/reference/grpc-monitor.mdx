...
Servers differ on how they answer for a name they do not know: the reference implementations return the `SERVICE_UNKNOWN` status, while some return the `NOT_FOUND` code. Both are reported as `SERVICE_UNKNOWN`.

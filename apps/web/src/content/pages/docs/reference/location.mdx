...
## Two forms of the same region code

The dashboard, the CLI, and the deprecated v1 REST API use the **bare** code (`iad`, `fra`,
`koyeb_fra`, `railway_us-east4-eqdc4a`). The ConnectRPC API, the Terraform provider, and the SDKs
use a **prefixed, hyphenated** form derived from the `Region` enum (`fly-iad`, `fly-fra`,
`koyeb-fra`, `railway-us-east4`).

They address the same probe — pick whichever form the surface you are using expects. The tables
below list both.

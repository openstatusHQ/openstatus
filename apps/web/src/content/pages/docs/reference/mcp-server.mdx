...
Get a key from the dashboard's **Settings > API Tokens**. When both credentials are present, the header wins.
...
Run `/mcp` inside Claude Code, select **openstatus**, and choose **Authenticate**. Your browser opens the consent screen; approve, and the tools appear. Step-by-step in [Connect openstatus to Claude Code](https://www.openstatus.dev/guides/connect-openstatus-to-claude-code).
...
To use an API key instead, add `"headers": { "x-openstatus-key": "{env:OPENSTATUS_API_KEY}" }` and `"oauth": false` to the entry. Step-by-step in [Connect openstatus to opencode](https://www.openstatus.dev/guides/connect-openstatus-to-opencode).
...
The first command writes the server to `~/.codex/config.toml`, which the Codex CLI, the Codex IDE extension, and the ChatGPT desktop app all share; the second opens the consent screen. For a headless setup, pass the API key instead: `--header "x-openstatus-key=os_…"`, or `env_http_headers = { "x-openstatus-key" = "OPENSTATUS_API_KEY" }` in a committed `.codex/config.toml`. Step-by-step in [Connect openstatus to Codex](https://www.openstatus.dev/guides/connect-openstatus-to-codex).

## Configure ChatGPT

In ChatGPT on the web, switch on **Developer mode** under **Settings > Security and login**, then open **Settings > Connectors > Create**, enter `https://api.openstatus.dev/mcp` as the MCP server URL with **OAuth** authentication, and complete the consent screen. Enable the connector per chat from the **+** menu under **Developer mode**. Step-by-step in [Connect openstatus to ChatGPT](https://www.openstatus.dev/guides/connect-openstatus-to-chatgpt).
...
Outside production they default to `http://localhost:3000` and `http://localhost:3001`.

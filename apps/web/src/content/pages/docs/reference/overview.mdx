...
- **[gRPC monitor](/docs/reference/grpc-monitor)** — health checks against the gRPC Health Checking Protocol.
...
- **[Notification channels](/docs/reference/notification)** — Slack, Discord, Microsoft Teams, Email, SMS, WhatsApp, Telegram, PagerDuty, OpsGenie, Google Chat, Grafana OnCall, Ntfy, and generic webhooks.
...
- **[API rate limits](/docs/reference/api-rate-limits)** — per-key and per-IP limits, the `429` response, and how to retry.
- **[API errors](/docs/reference/api-errors)** — every error code and reason, the v1 envelope, and the `ErrorInfo` detail on `/rpc`.

...
- Manage HTTP, TCP, DNS, and ICMP monitors with assertions.
...
- Provision private locations and assign monitors to them.
...
<Aside>
gRPC monitors are not yet exposed as a Terraform resource. Manage them from the dashboard, the
[ConnectRPC API](https://api.openstatus.dev/openapi), or the CLI until the provider catches up.
</Aside>
...
| `method`           | `string`        | No       | `"GET"`  | HTTP method: `GET`, `POST`, `HEAD`, `PUT`, `PATCH`, `DELETE`, `OPTIONS`, `TRACE`, `CONNECT`. |
...
- **`open_telemetry`** — Export this monitor's metrics to an OTLP endpoint (HTTP only).
  - `endpoint` (string, optional) — OTLP metrics endpoint URL.
  - `headers` (block list, optional) — Headers sent with each export; each has a required `key` and `value`.

**Read-only attributes:** `id` (string), `status` (string: `active`, `degraded`, `error`, `unknown`), `private_location_ids` (set of string — managed from `openstatus_private_location.monitor_ids`).
...
**Read-only attributes:** `id` (string), `status` (string), `private_location_ids` (set of string — managed from `openstatus_private_location.monitor_ids`).
...
Manages an ICMP (ping) monitor that checks host reachability and round-trip latency.
...
| :------------ | :------------ | :------- | :------- | :----------------------------------------------------------------- |
| `name`        | `string`      | Yes      |          | Monitor name (max 256 chars).                                      |
| `uri`         | `string`      | Yes      |          | Target host or IP address to ping, without a port.                 |
| `periodicity` | `string`      | Yes      |          | Check frequency: `30s`, `1m`, `5m`, `10m`, `30m`, `1h`.           |
| `timeout`     | `number`      | No       | `45000`  | Budget in milliseconds for the whole check (0–120000).             |
| `degraded_at` | `number`      | No       | computed | Average-latency threshold in milliseconds.                         |
| `retry`       | `number`      | No       | `3`      | Number of retries on a fully failed check (0–10).                  |
| `active`      | `bool`        | No       | `false`  | Whether the monitor is active.                                     |
| `public`      | `bool`        | No       | `false`  | Whether the monitor is publicly visible.                           |
| `description` | `string`      | No       |          | Monitor description (max 1024 chars).                              |
| `regions`     | `set(string)` | No       |          | Regions to monitor from. See [Available Regions](#available-regions). |
...
See the [ICMP monitor reference](/docs/reference/icmp-monitor) for how a check is run (three echo requests, average latency, packet loss).

**Example — Gateway reachability:**

```terraform
resource "openstatus_icmp_monitor" "gateway" {
  name        = "Gateway Ping"
  description = "Round-trip latency to the upstream gateway."
  uri         = "8.8.8.8"
  periodicity = "1m"
  timeout     = 10000
  active      = true
...
```bash
terraform import openstatus_icmp_monitor.gateway <monitor_id>
```
...
Manages a notification channel. Supports 12 provider types: Discord, Email, Slack, PagerDuty, OpsGenie, Webhook, Telegram, WhatsApp, Google Chat, Grafana OnCall, Microsoft Teams, and ntfy.
...
**Supported `provider_type` values:** `discord`, `email`, `slack`, `pagerduty`, `opsgenie`, `webhook`, `telegram`, `whatsapp`, `google_chat`, `grafana_oncall`, `ms_teams`, `ntfy`.
...
| `ms_teams`      | `webhook_url` (string, required, sensitive)                                                            |
...
| `access_type`        | `string`       | No       | Access control: `public`, `password`, `email-domain`, or `ip-restriction`.                  |
...
| `allowed_ip_ranges`  | `string`       | No       | Comma-separated IPv4 CIDR ranges. Required when `access_type` is `ip`.          |
| `theme`              | `string`       | No       | Theme key applied to the page.                                                 |
| `custom_theme`       | `object`       | No       | Per-mode CSS variable overrides merged over `theme`, with `light` and `dark` maps. Requires the custom-theme plan feature. |
| `default_locale`     | `string`       | No       | Default language for the page.                                                 |
| `locales`            | `list(string)` | No       | Locales offered in the language switcher.                                      |
| `allow_index`        | `bool`         | No       | Whether search engines may index the page.                                     |

**Read-only attributes:** `id` (string), `published` (bool), `created_at` (string), `updated_at` (string).
...
**Example — IP-restricted status page:**

```terraform
resource "openstatus_status_page" "internal" {
  title             = "Internal Status"
  slug              = "internal-status"
  access_type       = "ip-restriction"
...
Terraform owns `monitor_ids` and `metadata` — removing either from the configuration clears it on the next apply. Liveness (`status`, `last_seen_at`) is only available on the `openstatus_private_location` data source.

**Arguments:**

| Argument      | Type          | Required | Description                                                  |
| :------------ | :------------ | :------- | :----------------------------------------------------------- |
| `name`        | `string`      | Yes      | Display name for the private location.                        |
| `monitor_ids` | `set(string)` | No       | IDs of the monitors this private location runs.                |
| `metadata`    | `map(string)` | No       | User-defined key/value labels attached to this location.       |

**Read-only attributes:** `id` (string), `token` (string, sensitive), `created_at` (string), `updated_at` (string).
...
Look up a single monitor by ID. Works for HTTP, TCP, DNS, and ICMP monitors.
...
**Computed attributes:** `type` (`http`, `tcp`, `dns`, `icmp`), `name`, `url` (HTTP only), `uri` (TCP/DNS/ICMP only), `periodicity`, `method` (HTTP only), `active`, `public`, `description`, `timeout`, `status`, `private_location_ids`.
...
Look up a single private location by ID. Unlike the resource, this data source exposes agent liveness.
...
output "agent_last_seen" {
  value = data.openstatus_private_location.eu_dc.last_seen_at
}
```

**Computed attributes:** `name`, `monitor_ids`, `metadata`, `status` (`active`, `error`, or `unknown`), `last_seen_at` (RFC 3339, null if the agent has never reported), `token` (sensitive), `created_at`, `updated_at`.

---

### `openstatus_private_locations`

List all private locations with pagination.
...
**Computed:** `private_locations` — list of objects with `id`, `name`, `metadata`, `status`, `last_seen_at`, `monitor_count`, `created_at`, `updated_at`. Agent tokens are not returned by the list endpoint; use the singular data source to fetch one.
...
- **[HTTP Monitor Reference](/docs/reference/http-monitor)** — Detailed specification for HTTP monitor configuration.
- **[TCP Monitor Reference](/docs/reference/tcp-monitor)** — Detailed specification for TCP monitor configuration.
- **[DNS Monitor Reference](/docs/reference/dns-monitor)** — Detailed specification for DNS monitor configuration.
- **[CLI Reference](/docs/reference/cli-reference)** — Manage monitors using the openstatus command-line interface.

...
**Available for:** `POST`, `PUT`, `PATCH`, `DELETE` methods
...
The data URI is only decoded for `POST` requests that set `Content-Type: application/octet-stream`. With any other method or content type the string is sent literally.
...
**Note:** openstatus automatically adds `User-Agent: OpenStatus/1.0` to all requests. You can override it with a custom `User-Agent` header.

Up to 20 headers per monitor.

### Regions

**Type:** Array of strings (optional)
**Default:** a plan-based selection (4 regions on the free plan, 6 on paid plans)
...
[pricing](https://www.openstatus.dev/pricing) for the full breakdown.
</Aside>
...
Whether the checker follows HTTP redirects. When enabled, up to 10 redirects are followed and assertions run against the final response. When disabled, the first response is recorded as-is, so a `301`/`302` is what your status code assertions see.

### Assertions

Assertions let you validate specific aspects of the HTTP response. You can define up to 10 status code, 10 body, and 10 header assertions per monitor. Body assertions come in two flavours: raw text and JSONPath.
...
- `Not Empty` — the response body must not be empty.
- `Greater Than` / `Greater Than or Equal` — lexicographic comparison against the target.
- `Less Than` / `Less Than or Equal` — lexicographic comparison against the target.

#### JSON body assertions

Validate a single value inside a JSON response body, addressed by a
[JSONPath](https://www.npmjs.com/package/jsonpath-plus) expression. The extracted value is
compared as a string, using the same comparators as body assertions.

| Field | Description |
|---|---|
| `path` | JSONPath expression, e.g. `$.status` or `$.data.items[0].id`. |
| `compare` | Any body-assertion comparator. |
| `target` | Expected value. |

A body that fails to parse as JSON fails the assertion.

<Aside type="caution">
JSON body assertions cannot currently be created from any surface. The checker evaluates them,
and a monitor that already stores one keeps working, but none of the write paths accept one:
the dashboard's assertion editor offers status, header, and (text) body assertions only; the v1
REST API leaves the JSON body variant out of its assertion union; and the ConnectRPC API,
the SDKs, and the Terraform provider have no equivalent message.

One consequence is worth knowing if you hold an existing JSON body assertion: reading such a
monitor back over ConnectRPC reports it as a plain body assertion, with the JSONPath dropped.
The stored assertion itself is unchanged and the checker still evaluates the path.
</Aside>

#### Status code assertions

Validate the HTTP status code of the response. The target must be a valid status code, 100–599.
...
- `Not Empty` — a header's value must not be empty.
- `Greater Than` / `Greater Than or Equal` — lexicographic comparison against the target.
- `Less Than` / `Less Than or Equal` — lexicographic comparison against the target.

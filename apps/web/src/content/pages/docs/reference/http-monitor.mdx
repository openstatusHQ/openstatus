...
<Aside>
JSON body assertions are available through the REST and ConnectRPC APIs, the CLI, and Terraform.
The dashboard's assertion editor currently offers status, header, and (text) body assertions
only — a monitor created with a JSON body assertion keeps it, but you cannot add one from the UI.
</Aside>

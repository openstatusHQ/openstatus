...
- **Resolved** — either auto-resolved (`autoResolved: true`) or manually resolved, setting `resolvedAt` and `resolvedBy`.

Auto-resolution fires when the monitor leaves the `error` state by the same region-agreement rule
that opened the incident. That includes a transition to `degraded`, not only to `active`: a monitor
that improves from down to slow closes its open incident. The notification that goes out matches the
new state — a recovery alert for `active`, a degraded alert for `degraded`.

...
| **Time** | ~5 minutes |
| **Level** | Beginner |
| **Prerequisites** | openstatus account, one monitor (see [Create your first monitor](/docs/tutorial/create-your-first-monitor)), an email address you can read right now |
...
Open **Notifications** in the dashboard sidebar and click **Create Notification Channel**.
...
Wait a couple of minutes. Once at least half of the monitor's configured regions report an `error` (a single-region monitor triggers on its one region — see [Incident Reference](/docs/reference/incident)), openstatus opens an incident and fires the notification.
...
<Aside type="tip">
The same region agreement that fires the alert also gates *recovery*. If you point the test monitor back at a healthy URL, you'll get a second email confirming the monitor is up again, once the regions report success again.
</Aside>
...
- **[Set up the Slack agent](/docs/guides/how-to-setup-slack-agent)** — turn alerts into a two-way conversation where you can also create and resolve incidents from Slack.

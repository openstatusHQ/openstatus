...
Which is the argument for not running this by hand. [openstatus uptime monitoring](/uptime-monitoring) runs the same JSON-RPC handshake on a schedule from 28 regions and alerts you when `initialize` starts failing, `tools/list` comes back empty, or the protocol version shifts under you — the failures your AI clients hit first and report last.

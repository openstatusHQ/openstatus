...
faq:
  ...
  - question: "Can the assistant accidentally notify subscribers?"
    answer: "No. Every publishing tool requires the assistant to set notify: true | false explicitly — there's no default. Metadata-only edits like update_status_report can't notify at all. Every mutation lands in the audit log under actor_type = 'mcp', so you can see exactly what happened."
---
...
The server exposes **23 tools**, grouped by entity:
...
- **Content** *(public openstatus.dev pages)* — `search_docs` · `get_doc_page` · `search_content` · `get_content_page`

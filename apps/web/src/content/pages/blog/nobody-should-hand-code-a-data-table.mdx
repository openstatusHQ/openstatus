...
With the [shadcn](https://ui.shadcn.com?referrer=openstatus) components registry and [agent skills](https://docs.anthropic.com/en/docs/claude-code/skills?referrer=openstatus) — the pieces fit. We can ship code people own and an AI that knows how to set it up. No CLI installer needed. No npm package needed. And for AI coding tools, importing files via CLI keeps the context window lean — no need to overbloat it by copy-pasting entire repos worth of code.
...
The components aren't fully customizable in the builder (serialization constraints), but it helps getting started and understanding how schema changes affect the data table.
...
The [`/drizzle`](https://data-table.openstatus.dev/drizzle?referrer=openstatus) route shows it all working together: infinite scroll with cursor-based pagination, faceted search with live counts, nuqs URL state so filters are shareable, and time-bucketed charts via PostgreSQL's `date_bin()`.
...
The [shadcn registry](https://ui.shadcn.com/docs/registry?referrer=openstatus) solves the distribution problem. Adopting it required migrating to [Tailwind v4](https://tailwindcss.com/blog/tailwindcss-v4?referrer=openstatus) first — the registry blocks declare CSS variables using `@theme inline` syntax, so v4 was a prerequisite. Then we created a `registry.json` with 9 installable blocks:
...
It understands the project, installs the right blocks, generates a schema from the data model, wires up the state adapter, and configures the database integration - if you ask for. Start minimal and expand to your use case.

When something goes wrong? It doesn't print `Error: unknown flag --zustand`. It reads the error, understands the context, and fixes it.
...
The combination of shadcn registries and agent skills is a really good distribution model for frontend libraries. You don't publish a package with a fixed API. You ship composable code blocks that an AI understands how to assemble.
...
The full project is open source at [data-table-filters](https://github.com/openstatushq/data-table-filters?referrer=openstatus). Try the builder at [data-table.openstatus.dev/builder](https://data-table.openstatus.dev/builder?referrer=openstatus). Or just open your favorite chat interface and say "add a filterable data table" — the skill will take it from there.

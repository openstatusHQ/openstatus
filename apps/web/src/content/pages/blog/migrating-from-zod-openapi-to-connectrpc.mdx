...
description: "We killed our technical debt by merging two APIs into one. Here's how ConnectRPC gave us tRPC-level DX while keeping REST compatibility—and how AI cut our migration time from months to weeks."
...
**TL;DR:** Two APIs. Double the bugs. Double the work. We maintained tRPC for ourselves and REST for everyone else—until we found ConnectRPC. Now we have one schema-first API that's type-safe *and* curl-able. AI helped us finish in weeks instead of months.
...
Internally, we used **tRPC** for its end-to-end type safety. For our public API, we built a separate REST layer with **zod-openapi**.
...
OpenAPI can generate clients for multiple languages, but the tooling is hit-or-miss. For TypeScript, [Hey API](https://heyapi.dev/) is solid, but try generating a quality Go or Rust client and you'll spend hours fixing the output. We are actually experiencing it with for our CLI written in Go.
...
### 2. Protobuf Tooling That Doesn't Suck

The biggest fear with Protobuf? The tooling. The **Buf** ecosystem solved this.
...
Similar to Hono middleware, but with typed context and protovalidate integration out of the box.
...
## Before and After: Less Code, More Clarity

**Before (zod-openapi):** ~50 lines per endpoint, validation mixed with business logic.
...
The difference: validation lives in the schema, not scattered across Zod definitions and handler code. One source of truth for the contract.

---

## The Payoff: tRPC-Level DX for Everyone

This is where it all comes together. With `@connectrpc/connect-query`, we can get auto-generated TanStack Query hooks:
...
That's the "what." Here's the "how fast."

---

## How AI Cut Our Migration Time by 75%
...
We shifted from "writing boilerplate" to "reviewing code." A migration we estimated at months finished in weeks.
...
No more maintaining two APIs. We just build.

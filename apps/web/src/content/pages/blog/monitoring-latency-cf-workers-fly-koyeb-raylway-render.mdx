...
[website speed test](/play/checker) — the same 28 regions used for this benchmark,
with the per-phase timing breakdown, and no account needed.
...
In our benchmark, Cloudflare Workers had the lowest average latency at 182ms across 6 global regions, with a P75 of 138ms. Fly.io in production (with min_machines_running=1) averaged just 61ms, but with cold starts it averaged 1,471ms.

...
<div className="space-x-2">
  <ButtonLink href="https://app.openstatus.dev" children="Create Your Status Page" />
  <ButtonLink
    variant="ghost"
...
## Manage openstatus from your terminal, code, or AI agent

Every action in the dashboard is reachable programmatically. **One API key** works across an API, CLI, Node SDK, Terraform provider, and MCP server — and every mutation lands in the same audit log.

- **JSON-over-HTTP API** powered by ConnectRPC, with a published [OpenAPI spec](https://api.openstatus.dev/openapi)
- **CLI** with `--json` output, YAML config, and a GitHub Action for CI/CD
- **Node SDK** on [JSR](https://jsr.io/@openstatus/sdk-node) for TypeScript and Deno
- **Terraform provider** to version monitors as HCL — `openstatus terraform generate` bootstraps from your existing workspace
- **MCP server** so Claude, ChatGPT, Cursor, and other [Model Context Protocol](https://modelcontextprotocol.io) clients run your monitoring — every mutation requires an explicit `notify` flag
- **Agent skills** that wrap the CLI for Claude Code and other terminal-shaped agents

...
export const regionPercentile = Array.from({ length: 30 }, (_, i) => ({
  ...
  return Object.assign(item, {p50:Math.round(baseLatency*.7*randomFactor()),p75:Math.round(baseLatency*.85*randomFactor()),p90:Math.round(baseLatency*1.1*randomFactor()),p95:Math.round(baseLatency*1.3*randomFactor()),p99:Math.round(baseLatency*1.8*randomFactor()),error:[4,5,6].includes(i)?1:undefined});
});

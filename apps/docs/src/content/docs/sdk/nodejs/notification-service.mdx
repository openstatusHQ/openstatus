...
const { notification } =
  await client.notification.v1.NotificationService.createNotification({
    name: "Slack Alerts",
...
const { success, errorMessage } =
  await client.notification.v1.NotificationService.sendTestNotification({
    provider: NotificationProvider.SLACK,
    data: {
      data: {
        case: "slack",
...
const { notifications, totalSize } =
  await client.notification.v1.NotificationService.listNotifications({
    limit: 10,
    offset: 0,
  });
...
const { notification } =
  await client.notification.v1.NotificationService.getNotification({
    id: "notif_123",
  });
...
```typescript
const { notification } =
  await client.notification.v1.NotificationService.updateNotification({
    id: "notif_123",
...
```typescript
const { success } =
  await client.notification.v1.NotificationService.deleteNotification({
    id: "notif_123",
  });
```

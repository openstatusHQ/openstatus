...
You can customize the theme by adding `?theme=dark`, the size of it by adding `?size=lg` and the variant `?variant=outline`.
...
The **outline** variant simply adds a `stroke` to the svg `rect`. It mainly can be used for GitHub README or Markdown where styling is not supported.

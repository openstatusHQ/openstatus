...
You're running a Model Context Provider (MCP) server and need to ensure it's healthy and responsive. Traditional HTTP monitoring isn't sufficient because MCP servers use the JSON-RPC 2.0 protocol with specific request/response patterns.

## Solution

openstatus can monitor MCP servers by sending JSON-RPC ping requests to your endpoint. This guide shows you how to set up monitoring for any MCP server using the openstatus CLI.

## Prerequisites

-  An openstatus account and CLI installed
...
The [openstatus CLI](/tutorial/get-started-with-openstatus-cli) allows you to create monitor defined in a YAML configuration file. This is perfect solution if want to manage your monitors via GitOps.
...
You've successfully:
- ✅ Created a JSON-RPC based monitor for an MCP server
- ✅ Configured assertions to validate ping responses
- ✅ Set up monitoring from multiple global regions
- ✅ Automated monitor deployment using YAML configuration

## Next steps

- **[Run Synthetic Tests in GitHub Actions](/guides/how-to-run-synthetic-test-github-action/)** - Automate monitoring in your CI/CD
- **[Set up Alerting](/monitoring/customization/notification)** - Get notified when your MCP server goes down
- **[Export Metrics to OTLP](/guides/how-to-export-metrics-to-otlp-endpoint)** - Send metrics to your observability platform

## Related resources

- **[JSON-RPC 2.0 Specification](https://www.jsonrpc.org/specification)** - Understanding the protocol
- **[MCP Official Documentation](https://modelcontextprotocol.io/docs/concepts/architecture#debugging-and-monitoring)** - MCP health checks
- **[HTTP Monitor Reference](/reference/http-monitor)** - Complete API reference
- **[CLI Reference](/reference/cli-reference)** - Full CLI documentation

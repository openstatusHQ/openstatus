...
<Aside type="tip">
  You can customize the component name to be different from the monitor name.
  For example, your monitor might be named "prod-api-health-check" internally,
  but the component can display as "API Server" for your visitors.
</Aside>
...
<Aside type="caution">
  Static components **only** respond to manual status reports and scheduled
  maintenances. They never automatically detect issues or create incidents. If
  you need automatic failure detection, use a monitor component instead.
</Aside>
...
<Aside>
  Static components **never** generate incidents because they are not linked to
  monitors. Use status reports for manual issue tracking on static components.
</Aside>

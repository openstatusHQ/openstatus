...
| `list_maintenances`        | read     | List maintenance windows. Defaults to upcoming-only; pass `filter: "all"` for the full list. Response includes `truncated: boolean` so the LLM can warn when the result may not be exhaustive. |

...
We ingest the monitor response into the `ping_response__v4` table. If you want
...
It contains some dummy data that you can use to test the application. You still
need to update the `tenant_id` in table `user` with your user account from
Clerk.

Do the following to get your `tenant_id`:

![Sign up with clerk](/images/setup-env/clerk-step1.png)

Then go in the user section:

![Sign up with clerk](/images/setup-env/clerk-step2.png)

Then copy the `tenant_id`:

![Sign up with clerk](/images/setup-env/clerk-step3.png)

then update the `tenant_id` in the database, for this you can use
drizzle-studio:
